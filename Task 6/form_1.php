<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Document</title>
</head>
<body>


<table>
  <tr> 
    <td>username:</td>
    <td><input type="text" name="name"></td>
  </tr>
  <tr> 
    <td>Password:</td>
    <td><input type="text" name="password"></td>

  </tr>
  <tr> 
    <td>City of employment:</td>
    <td><input type="text" name="cityOfEmployment"></td>
  </tr>
  <tr> 
    <td>Web server:</td>
    <td>
    <select name="dog-names" id="dog-names"> 
        <option value="server one">server one</option> 
        <option value="server two">server two</option> 
        <option value="server three">server three</option> 
    </select>
  </td>
  <tr>   
     <tr>   
     <td>Please specify your role:</td>
    <td>
    <input type="radio" name="role" value="admin">
  <label for="html">Admin</label><br>
  <input type="radio" name="role value="engineer">
  <label for="engineer">Engineer</label><br>
  <input type="radio" name="role value="manager">
  <label for="manager">Manager</label><br>
  <input type="radio" name="role value="gest">
  <label for="gest">Gest</label><br>
    </td>
  </tr>
  </tr>
  <tr>   
     <tr>   
     <td>Sign in to the following</td>
    <td>
    <input type="checkbox" name="type1" value="mail">
  <label for="mail">Mail</label><br>
  <input type="checkbox" name="type2" value="payroll">
  <label for="payroll">Payroll</label><br>
  <input type="checkbox" name="type3" value="self-service">
  <label for="service">Self-service</label><br>
    </td>
  
</table>
<button>Login</button>  <button>Reset</button>

  </form>
 
</body>
</html>
